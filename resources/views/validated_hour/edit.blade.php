...
@section('content')
    <div class="container-fluid">
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-2 text-gray-800">Modifications des heures de {{$valid_hours->user->name}} , à la date
                du {{$valid_hours->date}}</h1>
        </div>
        <!-- Page Heading -->
        <div class="collapse-show" id="collapseCardExample">
            <div class="card-body">

                <form action="{{route('validated_hour.update', $valid_hours->id)}}" method="POST"
                      onsubmit="return fieldCondition()">
                    @csrf
                    <div class="input-group">
                        <input name="user_id" type="hidden" value="{{$valid_hours->user->id}}">
                        <select type="text" name="hour_id" id="hour_id" class="form-control bg-light border small"
                                oninput="verifierSomme()" value="{{old('time')}}"
                    ...
                    <p>Tâche 1</p>
                    <div class="input-group">
                        <input type="number" step="0.5" oninput="verifierSomme()" name="timer_one" id="timer_one"
                               class="form-control bg-light border small" value="{{$valid_hours->timer_one}}"
                        ...
                        <select type="text" name="stage_one" id="stage_one" class="form-control bg-light border small"
                                aria-label="Search" aria-describedby="basic-addon2">
                        ...
                        <input type="text" name="coment_one" class="form-control bg-light border small"
                               value="{{$valid_hours->coment_one}}"
                               placeholder="Commentaire" aria-label="Search" aria-describedby="basic-addon2">
                    </div>

                    <p>Tâche 2</p>
                    <div class="input-group">
                        <input type="number" step="0.5" oninput="verifierSomme()" name="timer_two" id="timer_two"
                               class="form-control bg-light border small" value="{{$valid_hours->timer_two}}"
                               placeholder="Temps de la tâche" aria-label="Search" aria-describedby="basic-addon2">

                        <select type="text" name="task_two" id="task_two" class="form-control bg-light border small"
                                aria-label="Search" aria-describedby="basic-addon2">
                        ...
                        <select type="text" name="stage_two" id="stage_two" class="form-control bg-light border small"
                                aria-label="Search" aria-describedby="basic-addon2">
                        ...
                        <input type="text" name="coment_two" class="form-control bg-light border small"
                               value="{{$valid_hours->coment_two}}"
                               placeholder="Commentaire" aria-label="Search" aria-describedby="basic-addon2">
                    </div>

                    <p>Tâche 3</p>
                    <div class="input-group">
                        <input type="number" step="0.5" oninput="verifierSomme()" name="timer_three" id="timer_three"
                               class="form-control bg-light border small" value="{{$valid_hours->timer_three}}"
                               placeholder="Temps de la tâche" aria-label="Search" aria-describedby="basic-addon2">
                        <select type="text" id="task_three" name="task_three" class="form-control bg-light border small"
                                aria-label="Search" aria-describedby="basic-addon2">
                        ...
                        <select name="subtask_three" id="subtask_three" class="form-control bg-light border small"
                                aria-label="Search" aria-describedby="basic-addon2">
                            <option
                                value="{{$valid_hours->subtaskThree->id}}">{{$valid_hours->subtaskThree->name}}</option>
                            @foreach($subtasks as $subtask)
                                <option value="{{$subtask->id}}">{{$subtask->name}}</option>
                            @endforeach
                        </select>
                        <input type="text" name="number_three" class="form-control bg-light border small"
                               value="{{$valid_hours->number_three}}"
                               placeholder="Numéro de l'OP" aria-label="Search" aria-describedby="basic-addon2">
                        <select type="text" name="project_three" id="project_three"
                                class="form-control bg-light border small" aria-label="Search"
                                aria-describedby="basic-addon2">
                            <option
                                value="{{$valid_hours->projectThree->id}}">{{$valid_hours->projectThree->name}}</option>
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}"> {{ $project->name }}</option>
                            @endforeach
                        </select>
                        ...
                        <input type="text" name="coment_three" class="form-control bg-light border small"
                               value="{{$valid_hours->coment_three}}"
                               placeholder="Commentaire" aria-label="Search" aria-describedby="basic-addon2">
                    </div>

                    <p>Tâche 4</p>
                    <div class="input-group">
                        <input type="number" step="0.5" oninput="verifierSomme()" name="timer_four" id="timer_four"
                               class="form-control bg-light border small" value="{{$valid_hours->timer_four}}"
                               placeholder="Temps de la tâche" aria-label="Search" aria-describedby="basic-addon2">
                        <select type="text" name="task_four" id="task_four" class="form-control bg-light border small"
                                aria-label="Search" aria-describedby="basic-addon2">
                        ...
                        <input type="text" name="number_four" class="form-control bg-light border small"
                               value="{{$valid_hours->number_four}}"
                               placeholder="Numéro de l'OP" aria-label="Search" aria-describedby="basic-addon2">
                        <select type="text" name="project_four" id="project_four"
                                class="form-control bg-light border small" aria-label="Search"
                                aria-describedby="basic-addon2">
                            <option
                                value="{{$valid_hours->projectFour->id}}"> {{$valid_hours->projectFour->name}}</option>
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}"> {{ $project->name }}</option>
                            @endforeach
                        </select>
                        <select type="text" name="stage_four" id="stage_four" class="form-control bg-light border small"
                                aria-label="Search" aria-describedby="basic-addon2">
                    ...
                    <div class="input-group-append">
                        <div>
                            ...
                            <button id="submitBtn" type="submit" class="btn btn-success btn-icon-split"
                                    spellcheck="false"><span class="icon text-white-50"><i
                ...
                @can('user_manage')
                    <form class="pt-3" action="{{route('validated_hour.destroy', $valid_hours->id)}}" method="POST">
                        @method("delete")
                        @csrf
                        <button id="submitBtn" type="submit" class="btn btn-danger btn-icon-split" spellcheck="false">
